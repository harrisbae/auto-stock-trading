...
def plot_results(df, results, title=None):
    """Plot equity curve and buy/sell signals"""
    ...
    for idx in results['buy_signals']:
        ax1.scatter(df.index[idx], df['Close'].iloc[idx], color='green', marker='^', s=100, alpha=0.7)
    
    for idx in results['sell_signals']:
        ax1.scatter(df.index[idx], df['Close'].iloc[idx], color='red', marker='v', s=100, alpha=0.7)
    
    # Plot equity curve on bottom chart
    ax2.plot(df.index, results['equity_curve'], label='Portfolio Value')
    
    # Set titles and labels
    strategy_params = results['parameters']
    ...
    main_title = title or f"{TICKER} Backtest Results - {params_str}"
    fig.suptitle(main_title, fontsize=12)
    
    performance_text = f"Total Return: {results['total_return']:.2f}% | Annual Return: {results['annual_return']:.2f}% | Max Drawdown: {results['max_drawdown']:.2f}%"
    ...
    if title:
        ...
        plt.savefig(f"backtest/results/{title.replace(' ', '_')}.png")
...
def run_parameter_comparison():
    """Run backtest with different parameter combinations and compare results"""
    # Download data once
    ...
    for i, params in enumerate(parameter_sets):
        ...
        plot_results(df, results, title=param_description)
    ...
    comparison_df.to_csv(f'backtest/results/{TICKER}_parameter_comparison.csv', index=False)
    
    # Plot comparative results
    plt.figure(figsize=(12, 8))
    plt.bar(range(len(comparison_data)), [data['total_return'] for data in comparison_data])
    plt.xlabel('Strategy Variant')
    plt.ylabel('Total Return (%)')
    plt.title(f'{TICKER} Strategy Variants Comparison')
    plt.grid(True, alpha=0.3)
    plt.savefig(f'backtest/results/{TICKER}_strategy_comparison.png')
    
    # Plot equity curves for top 5 strategies
    plt.figure(figsize=(15, 8))
    for i in range(min(5, len(all_results))):
        result_idx = [j for j, r in enumerate(all_results) if r['parameters'] == comparison_data[i]['parameters']][0]
        plt.plot(df.index, all_results[result_idx]['equity_curve'], 
                label=f"Strategy {i+1} ({comparison_data[i]['total_return']:.2f}%)")
    
    plt.xlabel('Date')
    plt.ylabel('Portfolio Value ($)')
    plt.title(f'Top 5 {TICKER} Strategies Equity Curves')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'backtest/results/{TICKER}_top_strategies.png')
    
    print(f"\nBacktest comparison completed. Results saved to backtest/results/{TICKER}_parameter_comparison.csv")
    print("Top 5 strategies by total return:")
    for i in range(min(5, len(comparison_data))):
        data = comparison_data[i]
        params = data['parameters']
        print(f"{i+1}. Total Return: {data['total_return']:.2f}%, Annual Return: {data['annual_return']:.2f}%, Max DD: {data['max_drawdown']:.2f}%")
