...
def check_trading_signal(ticker=None, notify_method='json_body', tranche_count=3, stop_loss_percent=7, 
                        band_riding_detection=True, risk_management_level="medium", use_mfi_filter=False):
    """
...
• 이격도: {current_dev_percent:.2f}%"""
        ...
        if 'risk_management' in trading_signal['data']:
            risk_msg = trading_signal['data']['risk_management'].get('message', '')
            if risk_msg:
                formatted_message += f"\n\n*[위험 관리 전략]*\n{risk_msg}"
        ...
        print(f"주식 데이터를 가져오는 데 오류가 발생했습니다. 종목: {actual_ticker}, 오류: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--notify_method", type=str, choices=['json_body', 'formatted_message'], help="알림 전송 방식")
    ...
    parser.add_argument("--band_riding_detection", type=bool, default=True, help="밴드타기 감지 여부")
    parser.add_argument("--risk_management_level", type=str, choices=['low', 'medium', 'high'], default="medium", help="위험 관리 수준")
    parser.add_argument("--use_mfi_filter", type=bool, default=False, help="MFI 필터 사용 여부")
